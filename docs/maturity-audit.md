...
2. Prove `activate-cloudflare-static` against the live recovered canary after deploying the new endpoint, then continue with hybrid-edge writeback or full Cloudflare-backed rollback only when Worker/KV/domain writes are coupled to strict live probes and audit events.
