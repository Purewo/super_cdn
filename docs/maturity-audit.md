...
- Cloudflare provider writes are not fully transactional with Super CDN metadata. A custom-domain propagation delay can make `deploy-site` verification time out after the Worker/domain write has already succeeded; `reconcile-deployment` can inspect recorded Super CDN metadata after a timeout, and `recover-cloudflare-static` can record verified unrecorded Cloudflare Static writes as non-active deployments with live canary coverage, but activation/re-activation of Cloudflare-backed deployments and hybrid-edge writeback are still not implemented as provider-aware writes.
...
2. Design provider-aware activation for recovered Cloudflare-backed deployments without using metadata-only `promote-deployment`; keep Cloudflare Static and hybrid-edge Worker/KV/domain writes coupled to strict live probes and audit events.
