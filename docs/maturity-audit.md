...
| Cloudflare Static recovery preflight | `supercdnctl recover-cloudflare-static` | Dry-run evidence validator for unrecorded Cloudflare Static provider writes. It summarizes the source directory, requires Worker/version/domain evidence, runs a strict Cloudflare Static live probe, and refuses `-dry-run=false` until a server-side recovery write/audit endpoint exists. |
...
- Cloudflare provider writes are not fully transactional with Super CDN metadata. A custom-domain propagation delay can make `deploy-site` verification time out after the Worker/domain write has already succeeded; `reconcile-deployment` can inspect recorded Super CDN metadata after a timeout, and `recover-cloudflare-static` can dry-run validate evidence for unrecorded Cloudflare Static writes, but the actual server-side recovery write/audit/activation path is not implemented yet.
...
2. Implement the server-side Cloudflare Static recovery write/audit endpoint from `docs/cloudflare-writeback-recovery-boundary.md`, then wire `recover-cloudflare-static -dry-run=false -confirm recover` to it.
