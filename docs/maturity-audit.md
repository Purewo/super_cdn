...
- Cloudflare Static rollback now has a local provider-aware `rollback-apply` write wrapper, but it still needs a live canary before production maturity can be claimed. Hybrid-edge rollback is still not implemented as a write operation; the future write-command boundary is recorded in `docs/cloudflare-rollback-boundary.md`.
- Cloudflare provider writes are not fully transactional with Super CDN metadata. A custom-domain propagation delay can make `deploy-site` verification time out after the Worker/domain write has already succeeded; `reconcile-deployment` can inspect recorded Super CDN metadata after a timeout, `recover-cloudflare-static` can record verified unrecorded Cloudflare Static writes as non-active deployments with live canary coverage, `activate-cloudflare-static` provides a live-verified dedicated activation path, and `rollback-apply` now covers Cloudflare Static source-verified rollback locally. Hybrid-edge writeback and hybrid-edge rollback writes are still not implemented as provider-aware writes.

## Next Concrete Work

1. Run a live Cloudflare Static `rollback-apply` canary against a disposable site and record deployment/audit/reconcile evidence before calling the Static rollback path production-proven.
2. Continue with hybrid-edge writeback or hybrid-edge rollback only when Worker/KV/domain writes are coupled to strict live probes and audit events.
