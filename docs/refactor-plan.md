...
- Documentation maturity follow-up: `go test ./cmd/supercdnctl` now checks that every dispatched `supercdnctl` command in `cmd/supercdnctl/main.go` is covered by both `docs/commands.md` and the built-in `usage()` output, so operator docs cannot silently miss newly added commands.
...
- Current narrow package-boundary extractions: `internal/deploymenttarget`, `internal/deploymentevidence`, `internal/edgeheaders`, `internal/cloudflarestatic`, and server audit action constants in `internal/server/audit.go`.
