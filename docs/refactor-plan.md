...
- Documentation maturity follow-up: `go test ./cmd/supercdnctl` now checks that every dispatched `supercdnctl` command in `cmd/supercdnctl/main.go` is covered by `docs/commands.md`, so the command book cannot silently miss newly added commands.
