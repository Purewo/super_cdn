...
When Super CDN generates or augments Cloudflare Static `_headers`, it adds `X-SuperCDN-Edge-Source: cloudflare_static` through a wildcard rule. This gives `probe-site -require-edge-static-html` a Super CDN-owned evidence header for direct Cloudflare Static traffic, including deployments that do not run the custom hybrid Worker path.
...
```http
POST /api/v1/objects/{id}/replicas/refresh
Content-Type: application/json
