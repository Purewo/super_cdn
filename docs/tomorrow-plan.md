...
5. Static asset bucket integration: edge URLs, purge by bucket/object, warmup, and health checks. In progress: overseas CDN bucket shortcut, upload `public_url`/`cdn_url`, upload-time warmup, and the asset-bucket list deadlock fix are implemented and live-tested.
