...
Current maturity snapshot: local foundation is green and recent pushed CI runs, including GitHub Actions run `25895224594`, were green for Go test, Ubuntu race, vet, vulnerability scan, command builds, OpenAPI lint, Worker tests, TypeScript check and Worker audit. Live Cloudflare rollback-path validation has now been run on canary domains: `supercdn-maturity-static-0515.qwk.ccwu.cc` proved Cloudflare Static A -> B -> redeploy A with strict probe and evidence headers, and `supercdn-maturity-hybrid-ipfs-0515.qwk.ccwu.cc` proved hybrid_edge A -> B -> redeploy A with active KV manifest writes and strict manifest-route probing. The earlier `repo_china_mobile` write-after-read visibility failure was reproduced on hybrid deployments, fixed by refreshing the AList/OpenList parent directory before visibility retry, deployed to production from commit `c2243727223d9ce9bf20a4692ff25797ec2c021e`, and revalidated with live deployment `dpl-diit34iw5d3t` on `supercdn-maturity-hybrid-0515.qwk.ccwu.cc`. The overall maturity goal is still not complete because Cloudflare Static `rollback-apply` needs live canary evidence, hybrid-edge rollback remains a read-only plan plus redeploy guidance, and Cloudflare provider writes are not fully transactional with CLI readiness timeouts. `deploy-site` verify failures now print provider write evidence plus retry/probe commands; `reconcile-deployment` can compare recorded Super CDN metadata with live provider behavior after a timeout; `recover-cloudflare-static` can record verified unrecorded Cloudflare Static writes as non-active deployments; `activate-cloudflare-static` is now the dedicated verified activation path for those recovered records and does not use generic `promote-deployment`; commits `249ebdc` and `bc08ede` deployed the recovery/activation endpoints to production with backups `/opt/supercdn/backups/20260515T010302Z-recovery-endpoint` and `/opt/supercdn/backups/20260515T013045Z-cf-static-activation`; live canary `supercdn-recovery-0515-090858.qwk.ccwu.cc` recorded and then activated deployment `dpl-diiuko109n5o`; `docs/cloudflare-writeback-recovery-boundary.md` records the remaining hybrid/rollback boundary; `rollback-plan` emits `rollback-apply` for Cloudflare Static when evidence is complete and post-redeploy `probe-site` commands for Cloudflare Static/hybrid-edge recovery; and `delete-deployment -dry-run` / API `dry_run=true` expose Cloudflare remote cleanup blockers instead of implying provider resources are removed. The remaining provider gap is hybrid-edge writeback / hybrid-edge rollback without metadata-only `promote-deployment`, plus live validation for the new Cloudflare Static rollback wrapper.

Release gate note: CI and `scripts/foundation-check.ps1` include PowerShell script syntax validation, `govulncheck`, GitHub Actions workflow linting, Redocly OpenAPI lint and Worker dependency audit as first-class checks. After pushing future changes, run `.\scripts\github-actions-status.ps1 -Wait -IncludeJobs` to verify the GitHub Actions run for the current branch/head SHA and include job/step summaries on failure. CI also runs `go test -race ./...` on Ubuntu; local Windows race coverage is available through `.\scripts\foundation-check.ps1 -SkipLinuxBuild -Race` only when a working C toolchain is installed. The module is pinned to Go `1.25.10` because older `go1.25.1` standard-library scans reported vulnerabilities. On this machine, use the temporary `http://127.0.0.1:10808` proxy for Go/npm downloads if direct access times out; if `proxy.golang.org` returns EOF during `govulncheck`, set `$env:GOPROXY="https://goproxy.cn,direct"` in that shell only.
...
cloudflare_static rollback-apply local wrapper: `rollback-apply` now loads the target deployment, verifies the local historical source directory against recorded Cloudflare Static assets hash/file count/size, defaults to dry-run, and with `-dry-run=false -confirm rollback` reruns the full Cloudflare Static provider deploy/record path instead of metadata-only `promote-deployment`. The record endpoint accepts `operation=rollback_apply` plus `rollback_target_deployment` and writes audit action `site.deployment.cloudflare_static.rollback`. Local tests cover dry-run verification, source mismatch blocking, confirmed write delegation and rollback audit recording. Next step is a disposable live canary before calling this production-proven.
