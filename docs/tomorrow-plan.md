...
Current maturity snapshot: local foundation is green and recent pushed CI runs, including GitHub Actions run `25891378394`, were green for Go test, Ubuntu race, vet, vulnerability scan, command builds, OpenAPI lint, Worker tests, TypeScript check and Worker audit. Live Cloudflare rollback-path validation has now been run on canary domains: `supercdn-maturity-static-0515.qwk.ccwu.cc` proved Cloudflare Static A -> B -> redeploy A with strict probe and evidence headers, and `supercdn-maturity-hybrid-ipfs-0515.qwk.ccwu.cc` proved hybrid_edge A -> B -> redeploy A with active KV manifest writes and strict manifest-route probing. The overall maturity goal is still not complete because Cloudflare Static / hybrid-edge rollback remains a read-only plan plus redeploy guidance, Cloudflare provider writes are not fully transactional with CLI readiness timeouts, and `repo_china_mobile` still has a write-after-read visibility failure mode on new hybrid deployment paths. `deploy-site` verify failures now print provider write evidence plus retry/probe commands; `rollback-plan` emits post-redeploy `probe-site` commands for Cloudflare Static and hybrid-edge recovery; and `delete-deployment -dry-run` / API `dry_run=true` expose Cloudflare remote cleanup blockers instead of implying provider resources are removed.
